...
const SchedulerCal = ({ dateType }) => {
    ...
    let um = new Date(2022,1,11,6,30)
    console.log(new Date(um))
    const ldata = useRef({

        dataSource:[
            
            {
            EndTime: um.toDateString(),
    ...
    onSnapshot(q,(snapshot)=>{
     ...
     ldata.current =   ldata.current.dataSource
        //setlocalData(localData.dataSource)
    ...
    return ( <>
        ...
        <ScheduleComponent   currentView={dateTypeView} popupClose={(e)=>{}}  actionComplete={(e)=>{updateAppointmentsToFirebase(e)}} actionBegin={()=>{}} showHeaderBar={false} height={'70vh'} width={'75vw'} eventSettings={ldata.current} >
