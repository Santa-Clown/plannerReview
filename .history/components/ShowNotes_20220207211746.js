...
const ShowNotes = ({ShowSingle}) => {
    ...
    useEffect(() => {
        ...
        const q = query(colRef, where('name','==','GEORGIOS MELADZE'), orderBy('timestamp', 'desc'))
        // getDocs(colRef)
        //     .then((snapshot) => {
    ...
    return (
        <div className={styles.container}>
            <div className={styles.notes} >
                ...
                {ar.map(note => (
                    <NoteList ShowSingle={ShowSingle} timeS={note.timeS}  key={note.id} id={note.id} name={note.name} message={note.message } />
