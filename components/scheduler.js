...
const SchedulerCal = ({ dateType ,setDatetoInfo }) => {
 ...
 <div className="time">To  :{props.EndTime.toLocaleString()}</div></div></div>);
        ...
        <ScheduleComponent  cellClick={(e)=>{setDatetoInfo.current([testState,e])}} timeFormat="HH:mm" startHour='08:00' endHour='22:00'   popupOpen={(e)=>{onPopupOpen(e)}} currentView={dateType} popupClose={(e)=>{}} actionComplete={updateAppointmentsToFirebase} enableAdaptiveUI ={true}    height={'75vh'} width={'100%'} eventSettings={{dataSource : testState.dataSource,     fields: {
                id: 'Id',
