...
const Nav = ({ s_out, dateTypeF, navField, searchWord, search, showAll, togglePin, toggler }) => {
    ...
    useEffect(() => {
        if (navField == 'add') {
            setNavfield(<NavShow toggler={toggler} showAll={showAll} SearchW={search} searchword={searchWord} />  //<NavAdd />
            )
        } else if (navField == 'show') {
            setNavfield(<NavShow togglePin={togglePin} showAll={showAll} SearchW={search} searchword={searchWord} />
