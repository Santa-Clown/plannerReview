...
const SchedulerCal = ({ dateType ,setDatetoInfo }) => {
 ...
 <div className="time">To  :{props.EndTime.toLocaleString()}</div></div></div>);
        ...
        <ScheduleComponent  cellClick={(e)=>{setDatetoInfo.current([testState,e])}} timeFormat popupOpen={(e)=>{onPopupOpen()}} currentView={dateType} popupClose={(e)=>{}} actionComplete={updateAppointmentsToFirebase} enableAdaptiveUI ={true}  actionBegin={()=>{}}  showHeaderBar={true} height={'75vh'} width={'100%'} eventSettings={testState}    >
