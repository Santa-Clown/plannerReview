...
const NoteList = ({ dateId, id,commentId,pinned, name, message,by, timeS, showSingle,admin, open, appointment , userName , createrName }) => {
    ...
    return (
       ...
       <> <a onDoubleClick={() => sendSingle()} href='#' className={styles.colums} style={
        ...
        <div className={styles.sepLine} ></div>
        <span style={{ overflow:'hidden'   }} className={styles.col3}>
            {/* <button >Σχόλια</button> */}
            {/* <textarea style={{border:'hidden'}}  name="comment" id="comment" cols="41" rows="10"></textarea> */}
            {/* {appointment.toUTCString()} */}
       
            <button style={{height:'100%' , width:'100%', background:'none', border:'none'}} onClick={()=>{window.prompt("sometext","defaultText");}}>
