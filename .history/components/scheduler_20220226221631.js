...
const SchedulerCal = ({ dateType ,setDatetoInfo }) => {
 ...
 <div className="time">To  :{props.EndTime.toLocaleString()}</div></div></div>);
        ...
        <ScheduleComponent  cellClick={(e)=>{setDatetoInfo.current([testState,e])}} timeFormat="HH:mm" workHours={{
            highlight: true, start: '9:00', end: '20:00'
        }}   popupOpen={(e)=>{onPopupOpen(e)}} currentView={dateType} popupClose={(e)=>{}} actionComplete={updateAppointmentsToFirebase} enableAdaptiveUI ={true}    height={'75vh'} width={'100%'} eventSettings={{dataSource : testState.dataSource,     fields: {
                id: 'Id',
                subject: { name: 'Subject', title: 'Όνομα' },
                location: { name: 'Location', title: 'Τοποθεσία ' },
                description: { name: 'Description', title: 'Περιγραφή' },
                startTime: { name: 'StartTime', title: 'Έναρξη' },
                endTime: { name: 'EndTime', title: 'Τέλος' }
            }}}    >
